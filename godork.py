...
class Googledork:
    ...
    user_agents = [
        ...
        "(Linux; U; Android 1.5; en-gb; HTC Magic Build/CRB17) AppleWebKit/528.5+ (KHTML, like Gecko) Version/3.1.2 Mobile Safari/525.20.1",
        ...
        "(X11; U; SunOS i86pc; en-US; rv:1.7) Gecko/20070606",
        "(Macintosh; U; Intel Mac OS X 10.5; en-US; rv:1.9.0.5) Gecko/2008120121 Firefox/3.0.5"
    ]
    ...
    def isocode(self):
        html = requests.get("https://www.google.com", headers={"User-Agent": random.choice(self.user_agents)})
    ...
    async def run(self):
        ...
        for i in range(int(self.page)):
            html = await self.aiorequest(self.base_url, params=self.params(self.query, str(i*10), hl=isocode if not self.hl else self.hl, gl=isocode if not self.gl else self.gl), headers={"User-Agent": random.choice(self.user_agents)})
